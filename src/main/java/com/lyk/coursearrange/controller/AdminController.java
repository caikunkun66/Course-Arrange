...
import com.lyk.coursearrange.common.PassToken;
...
public class AdminController {
    ...
    public ServerResponse test() {
        return ServerResponse.ofSuccess("后端服务正常运行！当前时间：" + new java.util.Date());
    }
    ...
    @PassToken
    @GetMapping("/test-db")
    public ServerResponse testDb() {
        try {
            long count = adminService.count();
            return ServerResponse.ofSuccess("数据库连接正常！管理员数量：" + count);
        } catch (Exception e) {
            return ServerResponse.ofError("数据库连接失败：" + e.getMessage());
        }
    }
