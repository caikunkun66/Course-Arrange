...
public class StudentController {
    ...
    public ServerResponse joinTeacher(@PathVariable("id") Integer id, @PathVariable("teacherId") Integer teacherId) {
        ...
        if (b) {
            return ServerResponse.ofSuccess("加入成功");
        }
        return ServerResponse.ofError("加入失败");
    }
    ...
    public ServerResponse studentLogin(@RequestBody StudentLoginRequest studentLoginRequest) {
        ...
        // 先判断是否有该昵称，该学生
        QueryWrapper<Student> wrapper = new QueryWrapper<Student>().eq("username", studentLoginRequest.getUsername());
        // 查询是否有该学生
    ...
    public ServerResponse queryStudent(@PathVariable("page") Integer page,
        ...
        QueryWrapper<Student> wrapper = new QueryWrapper<Student>().orderByDesc("create_time");
    ...
    public ServerResponse queryStudentsByTeacher(@PathVariable("teacherId") Integer teacherId,
    ...
    public ServerResponse addStudent(@RequestBody Student student) {
        // 检查昵称是否已存在
        QueryWrapper<Student> wrapper = new QueryWrapper<Student>().eq("username", student.getUsername());
        Student existStudent = studentService.getOne(wrapper);
        if (existStudent != null) {
            return ServerResponse.ofError("昵称已存在");
        }
        
        boolean b = studentService.save(student);
        if (b) {
            return ServerResponse.ofSuccess("添加学生成功", student);
        }
