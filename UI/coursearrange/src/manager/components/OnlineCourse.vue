...
export default {
  ...
  methods: {
    init() {
      this.options = [];
      this.$axios.get("/category/one").then(r => {
        let c = r.data.data;
        c.map(v => {
          this.$axios
            .get("/category/two/" + v.id)
            .then(rr => {
              this.options.push({
                id: v.id,
    ...
    handleVideoDelete(row) {
      this.$axios
        .delete("/onlinevideo/delete/" + row.id)
        .then(r => {
          if (r.data.code == 0) {
    ...
    remoteMethod() {
      ...
      this.$axios
        .get("/onlinecourse/list/" + 1, params)
        .then(r => {
