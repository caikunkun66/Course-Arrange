...
export default {
  ...
  methods: {
    ...
    searchCourse() {
      this.page = 1
      this.$axios
        .get("/courseinfo/search/" + this.page + "/" + this.keyword)
        .then(res => {
    ...
    addInfo() {
      ...
      this.visibleForm = true
      // this.editFormData = {}
      this.$axios.get("/courseinfo/get-no")
      .then(res => {
        if (res.data.code == 0) {
          // 获取课程编号
    ...
    // 添加教材
    add() {
      this.$axios.post("/courseinfo/add", this.editFormData)
      .then(res => {
        if (res.data.code == 0) {
          this.allCourseInfo()
    ...
    modifyCourseInfo(modifyData) {
      this.$axios
        .post("/courseinfo/modify/" + this.editFormData.id, modifyData)
        .then(res => {
