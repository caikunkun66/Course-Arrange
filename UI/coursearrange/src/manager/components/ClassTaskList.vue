      ...
      <div>
        ...
        <div>
          ...
          >
            ...
            <!-- 下载模板 <a class="atag" href="/download">-->
...
export default {
  ...
  methods: {
    ...
    // 提交添加
    commit() {
      this.$axios.post("/addclasstask", this.addClassTaskForm)
      .then(res => {
        if (res.data.code == 0) {
          // 添加完成
          this.allClassTask()
    ...
    // 点击开始提交学期到系统后台排课
    arrangeCourse() {
      this.$axios.post("/arrange/" + this.semester)
      .then(res => {
        if (res.data.code == 0) {
          this.allClassTask()
    ...
    allClassTask() {
      this.$axios
        .get(
          "/classtask/" + this.page + "/" + this.semester
        )
        .then(res => {
          let ret = res.data.data
